...
#include "globals_time.h"
    //-- same variables as P5
    //-- delay duration is increased (3000 ms)
    //-- led trial duration is adjusted (1500 ms)
...
#include "IR.h" //-- basically the poke counter; moves you to trial phase if M pokes during initiation window
    //-- Poke in L or R port --> will register as poke if IR turned off
    // poke in middle port --> will register as poke out if IR turned off
        //-- if it was in the trial initiation phase: (otherwise it will just count the number of middle pokes)
...
#include "begin_trial.h" //-- takes you through different windows
    //-- after iti window or in the first trial, the parent if will become True (OTHERWISE THIS WON'T RUN)
    ...
    //-- if trial_initiation_window is  true, turn on M LED if off, on if off (appears as blinking!)
    //-- once there is a poke in M port, valid_trial_window will be true (IR.h) so we will move to the "trial window" code
        //-- all lights will turn off, time will be recorded, initiate delay will be true
    //-- if initiate_delay is true:
        //-- when delay duration has been exceeded:
            //-- everything will be reset afterwards
            //-- print the number of pokes during this time (counted by IR.h) into the CSV file
...
void setup(){
   ...
   for (byte i = 11; i <=13; i++) { pinMode(i, INPUT);}
    ...
    Serial.print("Delay_Window:"); //-- prints how long the delay is (I WILL NEED TO SEPARATE THIS TIME FOR L AND R)
